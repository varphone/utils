#/bin/bash
#
# Script for create custom cross toolchain for Ambarella A5S platform
...
TARGET=arm-a5s-linux-gnueabi
TARGET_ROOT=${HOME}/workspace/toolchain/${TARGET}
TARGET_SYSROOT=${TARGET_ROOT}/${TARGET}/sysroot
#TARGET_CFLAGS="-Os -g -pipe -mlittle-endian -march=armv6k -mcpu=arm1136j-s -msoft-float"
#TARGET_CXXFLAGS="-Os -g -pipe -mlittle-endian -march=armv6k -mcpu=arm1136j-s -msoft-float"
...
PKG_VERSION="VCT v100 for Ambarella A5S(gcc-4.8.0,glibc-2.16,eabi,ntpl)"
    ...
    [ -f .$1.$2.${TARGET}.ok ] || {
    	...
    	case "$2" in
	    	stage1)
	    		...
	    		;;
	    	stage2)
	    		CC_FOR_BUILD=${HOST}-gcc CFLAGS="${HOST_CFLAGS} ${HOST_HEADERS}" CXXFLAGS="${HOST_CXXFLAGS} ${HOST_HEADERS}" LDFLAGS="${HOST_LDFLAGS} ${HOST_LIBS}" ../$1/configure --build=${HOST} --host=${HOST} --target=${TARGET} --prefix=${TARGET_ROOT} --enable-shared --enable-threads=posix --disable-multilib --disable-isl-version-check --enable-languages=c,c++ --enable-__cxa_atexit --with-arch=armv6k --with-cpu=arm1136j-s --with-float=soft --with-local-prefix=${TARGET_SYSROOT} --with-sysroot=${TARGET_SYSROOT} --with-host-libstdcxx="-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm" --with-pkgversion="${PKG_VERSION}" --with-bugurl="${BUGURL}" --without-newlib || exit 1
	    		make -j4 -l all-gcc all-target-libgcc || exit 1
	    		make -j4 -l install-gcc install-target-libgcc || exit 1
	    		;;
	    	stage3)
	    		CC_FOR_BUILD=${HOST}-gcc CFLAGS="${HOST_CFLAGS} ${HOST_HEADERS}" CXXFLAGS="${HOST_CXXFLAGS} ${HOST_HEADERS}" LDFLAGS="${HOST_LDFLAGS} ${HOST_LIBS}" CFLAGS_FOR_TARGET="${TARGET_CFLAGS} " CXXFLAGS_FOR_TARGET="${TARGET_CXXFLAGS}" LDFLAGS_FOR_TARGET="${TARGET_LDFLAGS}" ../$1/configure --build=${HOST} --host=${HOST} --target=${TARGET} --prefix=${TARGET_ROOT} --disable-multilib --disable-isl-version-check --enable-shared --enable-threads=posix --enable-target-optspace --enable-languages=c,c++ --enable-__cxa_atexit --with-arch=armv6k --with-cpu=arm1136j-s --with-float=soft --with-local-prefix=${TARGET_SYSROOT} --with-sysroot=${TARGET_SYSROOT} --with-host-libstdcxx="-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm" --with-pkgversion="${PKG_VERSION}" --with-bugurl="${BUGURL}" --without-newlib || exit 1
	    		make -j4 -l || exit 1
...
build_linux_headers linux-2.6.38
...
build_target_busybox busybox-1.21.0 a5s_defconfig
