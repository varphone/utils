#/bin/bash
#
# Script for create custom cross toolchain for Ti DM368 platform
...
#   $ mkdir xxx
#   $ cd xxx
#   $ bld-arm-dm368-linux-gnueabi.sh
#
...
TARGET=arm-dm368-linux-gnueabi
TARGET_ROOT=${HOME}/workspace/toolchain/${TARGET}
TARGET_SYSROOT=${TARGET_ROOT}/${TARGET}/sysroot
#TARGET_CFLAGS="-Os -g -pipe -mlittle-endian -march=armv5te -mcpu=arm926ej-s -msoft-float"
#TARGET_CXXFLAGS="-Os -g -pipe -mlittle-endian -march=armv5te -mcpu=arm926ej-s -msoft-float"
...
PKG_VERSION="VCT v100 for Ti DM368(gcc-4.8.0,glibc-2.16,eabi,ntpl)"
...
build_gcc()
{
    ...
    [ -f .$1.$2.${TARGET}.ok ] || {
    	...
    	case "$2" in
	    	stage1)
	    		CC_FOR_BUILD=${HOST}-gcc CFLAGS="${HOST_CFLAGS} ${HOST_HEADERS}" CXXFLAGS="${HOST_CXXFLAGS} ${HOST_HEADERS}" LDFLAGS="${HOST_LDFLAGS} ${HOST_LIBS}" ../$1/configure --build=${HOST} --host=${HOST} --target=${TARGET} --prefix=${TARGET_ROOT} --disable-shared --disable-threads --disable-multilib --disable-isl-version-check --enable-languages=c,c++ --enable-__cxa_atexit --enable-target-optspace --with-float=soft --with-newlib --with-libelf=${HOST_SYSROOT} --with-local-prefix=${TARGET_SYSROOT} --with-sysroot=${TARGET_SYSROOT} --with-host-libstdcxx="-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm" --without-headers || exit 1
	    		make -j4 -l all-gcc all-target-libgcc || exit 1
	    		make -j4 -l install-gcc install-target-libgcc || exit 1
	    		;;
	    	stage2)
	    		CC_FOR_BUILD=${HOST}-gcc CFLAGS="${HOST_CFLAGS} ${HOST_HEADERS}" CXXFLAGS="${HOST_CXXFLAGS} ${HOST_HEADERS}" LDFLAGS="${HOST_LDFLAGS} ${HOST_LIBS}" ../$1/configure --build=${HOST} --host=${HOST} --target=${TARGET} --prefix=${TARGET_ROOT} --enable-shared --enable-threads=posix --disable-multilib --disable-isl-version-check --enable-languages=c,c++ --enable-__cxa_atexit --with-arch=armv5te --with-cpu=arm926ej-s --with-float=soft --with-local-prefix=${TARGET_SYSROOT} --with-sysroot=${TARGET_SYSROOT} --with-host-libstdcxx="-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm" --with-pkgversion="${PKG_VERSION}" --with-bugurl="${BUGURL}" --without-newlib || exit 1
	    		make -j4 -l all-gcc all-target-libgcc || exit 1
	    		make -j4 -l install-gcc install-target-libgcc || exit 1
	    		;;
	    	stage3)
	    		CC_FOR_BUILD=${HOST}-gcc CFLAGS="${HOST_CFLAGS} ${HOST_HEADERS}" CXXFLAGS="${HOST_CXXFLAGS} ${HOST_HEADERS}" LDFLAGS="${HOST_LDFLAGS} ${HOST_LIBS}" CFLAGS_FOR_TARGET="${TARGET_CFLAGS} " CXXFLAGS_FOR_TARGET="${TARGET_CXXFLAGS}" LDFLAGS_FOR_TARGET="${TARGET_LDFLAGS}" ../$1/configure --build=${HOST} --host=${HOST} --target=${TARGET} --prefix=${TARGET_ROOT} --disable-multilib --disable-isl-version-check --enable-shared --enable-threads=posix --enable-target-optspace --enable-languages=c,c++ --enable-__cxa_atexit --with-arch=armv5te --with-cpu=arm926ej-s --with-float=soft --with-local-prefix=${TARGET_SYSROOT} --with-sysroot=${TARGET_SYSROOT} --with-host-libstdcxx="-static-libgcc -Wl,-Bstatic,-lstdc++,-Bdynamic -lm" --with-pkgversion="${PKG_VERSION}" --with-bugurl="${BUGURL}" --without-newlib || exit 1
	    		make -j4 -l || exit 1
...
build_linux_headers linux-2.6.32.17
...
build_target_busybox busybox-1.21.0 dm368_defconfig
...
(
    ...
    find . -type f -exec bash -c "ln -s {} \$(echo {} | sed 's/arm-dm368-linux-gnueabi/arm-linux/')" \; || exit 1
    popd
) || exit 1
